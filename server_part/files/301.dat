...
public winCaption as string="Dialog für Anwendung XYZ"
...
Dim pLeft4, pRight4 As ScriptedPanel
 ...
 dim jumboBgColor as string="#E6C768"
 dim jumboForeColor as string="#111"
...
'-->
sub test1(optional dummy as object=nothing)
  'msgBox("test1")
  printLine 3, "aaa","bbbbb"
  monitorAdd("---------------------------------------")
  dim container  As ScriptedPanel=pRight33
  dim fileSpec as string=glob_defaultGridPath+"esGrid1.txt"
  dim DATA as string="LEER"
  createDialogfromData(container, fileSpec, data)
end sub
...
Sub AutoStart()
  ...
  '--> ... --- Left Pane - 1
  with pLeft1
    .backColor=ColorTranslator.FromHtml("#E0BB48")
    .resetControls (5,5)
 ...
 '--> ... container_2  :OBEN / UNTEN
  ...
  with pLeft3
    ...
    .backColor=ColorTranslator.FromHtml("#E0BB48")
    .insertX = 11 :.insertY = 12
    .addIcon("appPicture33", "http://es.teamwiki.net/docs/icons/128email.png " )

    '.insertX = 15  :.insertY = 160
    .insertX = 190 :.insertY = 15
    dim label as string="T I T E L "
    el=.addLabel  ("labA_01", label ,  ,"",,,-2,35) :
    el.backColor=ColorTranslator.FromHtml(jumboBgColor): el.foreColor=ColorTranslator.FromHtml(jumboForeColor): 
    makeJumboLabel(el)

    label ="Überschrift..."+vbNewLine+vbNewLine+"... 2. Zeile"+vbNewLine+"... 3. Zeile"+vbNewLine+"... 4. Zeile"+vbNewLine+"... 5. Zeile"+vbNewLine+"... 6. Zeile"
    '.insertX = 15  :.insertY = 210
    .insertX = 190  :.insertY = 60
    el=.addLabel  ("labA_02", label ,  ,"",,,-2,35) :
    ...
    el.Padding = New System.Windows.Forms.Padding(5)
    el.height=77
   el.backColor=ColorTranslator.FromHtml(jumboBgColor): el.foreColor=ColorTranslator.FromHtml(jumboForeColor): 
...
'--> ... --- Left Pane - 2
 ...
 '--> ... container_5
  ...
  '--> !!! noch unfertig: 'setGridHeaderData'
  '' geht hier anders
  '' setGridHeaderData(igrid1,globGridHeaderText1)
...
sub IGrid1_updateUserFeedback2(ByRef grid As TenTec.Windows.iGridLib.iGrid, ByVal para1 As String) Handles iGrid1.updateUserFeedback2
  ...
  :   PanelRef.element("txt_dlg_"+i.toString).text=item
  : next 
...
sub refreshHelpLabels(e as object, index as string, value as string )
  ...
  monitorAdd(titel)
  dim el as object
  el=panelRef.element("lab_labA_01")
  ''msgbox(el.toString)
  el.text=titel
  el=panelRef.element("lab_labA_02")
  ''msgbox(el.toString)
  el.text=FIELDS(17)
end sub
