...
public globSkipTextboxEvents as boolean =false
...
sub test3()
  ...
  dim iNummer as string ="1111"
  iniDbReaderFromINummer(dbReader, inummer)
  
  monitorAdd("1", field(dbReader,"kurzanschrift"))
  monitorAdd("2", field(dbReader,"Name"))
  monitorAdd("3", field(dbReader,"vorname"))
  monitorAdd("4", field(dbReader,"ort"))
...
'--> --------------------------------------------------------
 ...
 '-->
 '--> auto(1):  A U T O S T A R T (global)
 
Sub AutoStart()

  '... die werden ganz am ende zugewiesen
  dim spliterDistanceTop=145
  dim pliterDistanceLeft1=220
  dim pliterDistanceLeft2=500
  ...
  with toolbar1
   ...
   .addButton ("cmdSwitchContainer_2"  , "Versand-Daten"        ) ' , "#1DD910")
...
#EndData
 ...
 '--> auto(2):   DATEN-EINFÜGEN 
  ...
  with cToolBar
    .visible=true
    .addButton ("cmdSplitAdrData_1"     , "in eMail und Post auftrennen" )' , "#1DD910")
 ...
 '--> auto(3+4):   LISTEN
  ...
  with cToolBar
    ...
    .height=33
    .resetControls (5,5)
    .addButton ("cmdSplitAdrData_2"     , "in eMail und Post auftrennen" , "#E4E1DC")
...
'--> createDialogfromIgrid
   createDialogfromIgrid(pRight33 ,1)
   createDialogfromIgrid(pRight44 ,2)
   createDialogfromIgrid(pRight55 ,3)
...
'-->IGrid1-width   
  ...
  '...der muß nach readFormPos stehen, sonst spinnt er

  splitContainer3.splitterDistance=spliterDistanceTop
  splitContainer5.splitterDistance=spliterDistanceTop
  splitContainer4.splitterDistance=spliterDistanceTop

  splitContainer33.splitterDistance=pliterDistanceLeft1
  splitContainer55.splitterDistance=pliterDistanceLeft2
  splitContainer44.splitterDistance=pliterDistanceLeft2 
...
Sub onTextBoxEvent(e)
  'exit sub '-->???
  if globSkipTextboxEvents = true then exit sub
  setMonitorRef()
  statustext("OK")
  errorText("")
...
:sub IGrid1_updateUserFeedback2(ByRef grid As iGridEx, ByVal para1 As String) _ 
    Handles iGrid1.updateUserFeedback2, _
            iGrid2.updateUserFeedback2, _
            iGrid3.updateUserFeedback2
  ...
  dim deltaIndex as integer=grid.deltaIndex
  MonitorAdd("Feedback2: " + deltaIndex.toString)
  ...
  : next 
  globSkipTextboxEvents=false

end sub


sub syncIGrid(e as object, index as string, value as string )
  '...wird durch 'onTextBoxEvent(e)' aufgerufen
  dim grid as iGridEx
  monitorAdd(index)

  grid=iGrid1
  if index>199 then grid=iGrid2
  if index>299 then grid=iGrid3
...
'-->
'-->HELPER: cls_configPara

''Shared globConfig As New cls_configPara()




Shared Function dbFileSpec_ADR() As String
  'dbFileSpec_ADR = configPara("db_adr")
  dim fileSpec as string
  fileSpec=globConfig.Para("DB_ADR")
  'filespec="...forTest"
  myScriptClass.monitorAdd("fileSpec", fileSpec)
  Return filespec
End Function
...
' Standard-Parameter für die Config-Datei
Shared Function DefaultValuesForConfigFile() As Dictionary(Of String, String)
  DefaultValuesForConfigFile = New Dictionary(Of String, String)
  With DefaultValuesForConfigFile
    '' .Add("KVS_KonzeptListe", "")
    '' .Add("KVS_AusgabeOrdner", "U:\yPara\kvs\eigeneBriefe\")
    '' .Add("DB_ADR", "")
  End With
End Function
...
'...der Ort der DatenBank wird aus dem globalen configFile geholt
...
Sub cmdSplitAdrData(e)  ' ...
  ...
  For i As Integer = 0 To max
    ir = Grid.Rows(i)
    iNummer= trim(ir.Cells(0).Value.ToString)
    if iNummer="id" or iNummer="ID"  or iNummer="" then continue for
    'monitorAdd(iNummer)
    iniDbReaderFromINummer(dbReader, iNummer)
    monitorAdd("kurzAdr", field(dbReader,"kurzanschrift"))

    email=trim(field(dbReader,"email"))
    if email="" then
      postMailListAdd(igrid3, iNummer, dbReader)
    else
      eMailListAdd(igrid2, iNummer, dbReader)
    end if
    '--------- nach getaner Arbeit: 
    dbReader.Close()
  Next
...
sub eMailListAdd(iGrid As IgridEx, iNummer as string, dbReader As OleDb.OleDbDataReader)
  dim row as igRow
  row=iGrid.Rows.add
  
  '----------------------------------------dbReader öffnen/ist schon offen
  'dim dbReader As OleDb.OleDbDataReader
  'dim iNummer as string ="1111"
  'iniDbReaderFromINummer(dbReader, inummer)
  dim iNum as integer=cInt(iNummer)
  dim iNum2 as string=iNum.toString("00000")
  
  dim vorname=trim(field(dbReader,"vorname"))
  if vorname<>"" then vorname=vorname+" "
  dim name as string = field(dbReader,"Name")
  dim anrede as string=trim(field(dbReader,"anrede"))
  if anrede="" then
    anrede="Sehr geehrte Damen und Herrn"
  else
    anrede="Sehr geehrte"+field(dbReader,"anrede")  '...lücke darf nicht rausfallen
  end if

  dim i as integer=-1
  i=i+1: row.cells(i).value=iNum2
  i=i+1: 
  i=i+1: 
  i=i+1: row.cells(i).value=field(dbReader,"eMail")
  i=i+1: row.cells(i).value=vorname+name
  i=i+1: row.cells(i).value=anrede
  'i=i+1: row.cells(i).value=field(dbReader,"ort")

  '--------- nach getaner Arbeit:  ... macht die aufrufende funktion
  'dbReader.Close()
end sub


sub postMailListAdd(iGrid As IgridEx, iNummer as string, dbReader As OleDb.OleDbDataReader)
  dim row as igRow
  row=iGrid.Rows.add
  ...
  i=i+1: row.cells(i).value=vorname+name
  i=i+1: row.cells(i).value=anrede
  i=i+1: row.cells(i).value=field(dbReader,"Ausland")
  i=i+1: row.cells(i).value=field(dbReader,"plzNeu")
  i=i+1: row.cells(i).value=field(dbReader,"A2")
  i=i+1: row.cells(i).value=field(dbReader,"A3")
  i=i+1: row.cells(i).value=field(dbReader,"A4")
  i=i+1: row.cells(i).value=field(dbReader,"A5")
  i=i+1: row.cells(i).value=field(dbReader,"A6")
  i=i+1: row.cells(i).value=field(dbReader,"A7")
  i=i+1: row.cells(i).value=field(dbReader,"A8")
...
'-->
'-->MODUL: DB-NEU  (...aus KVS)
Public conADR As OleDb.OleDbConnection


function field(dbReader As OleDb.OleDbDataReader, fieldName as string) as string
  dim content = dbReader(fieldName)
  If TypeOf content Is DBNull Then
    Return ""
  Else
    Return content
  End If
...
Sub iniDbReaderFromINummer(byRef dbReader As OleDb.OleDbDataReader,ByVal inummer As String, optional traceMode as boolean =false)
  '' dim dbReader as  As OleDb.OleDbDataReader
  '' dim iNummer as string
  '' '----
  '' '... nach getaner arbeit: read.Close()

  inummer = Trim(Replace(Trim(inummer), "i-", ""))
  If IsNumeric(inummer) = False Or inummer = "0" Then Exit Sub

  if conADR is nothing then openDBConnection()

  Dim sql as string
  sql = "SELECT * FROM [UGB-ADR-TAB] WHERE ID = " + inummer
  dbReader = dbGetReader(sql)
  If dbReader.HasRows = False Then 
     MsgBox("Unter i-" & inummer & " wurde kein Datensatz gefunden.")
     Exit Sub
  end if

  if traceMode=true then   myScriptClass.monitorAdd("----------------------------")
  if traceMode=true then   myScriptClass.monitorAdd("dbReader.read ... für:", inummer)
  
  dbReader.Read()

  if traceMode=true then   myScriptClass.monitorAdd("kurzanschrift", field(dbReader,"kurzanschrift"))
  if traceMode=true then   
     trace "kurzanschrift", field(dbReader,"kurzanschrift")
  end if
  '...dbReader.Close() '... hier nicht
  if traceMode=true then   myScriptClass.monitorAdd(" dbReader.Close() !!! nicht vergessen")


end sub

Sub navigateInummer(ByVal inummer As String)
  inummer = Trim(Replace(Trim(inummer), "i-", ""))
  If IsNumeric(inummer) = False Or inummer = "0" Then Exit Sub

  openDBConnection()
  Dim sql as string
  'sql = "SELECT name,vorname,kurzanschrift,SEM2,kvsDatum01,kvsDatum02,kvsDatum03,kvsDatum04,kvsDatum05,kvsDatum06,kvsKOM,kvsVersicherung FROM [UGB-ADR-TAB] WHERE ID = " + inummer
  sql = "SELECT * FROM [UGB-ADR-TAB] WHERE ID = " + inummer
  Dim read = dbGetReader(sql)
  ...
  read.Read()
  myScriptClass.monitorAdd("1", field(read,"kurzanschrift"))
  myScriptClass.monitorAdd("2", field(read,"Name"))
  myScriptClass.monitorAdd("3", field(read,"vorname"))
  myScriptClass.monitorAdd("4", field(read,"ort"))

  trace 1, field(read,"kurzanschrift")
  trace 2, field(read,"Name")
  trace 3, field(read,"vorname")
  trace 4, field(read,"ort")
  
  '' read.Close()  '... muß die aufrufende funktion machen
end sub



Function replaceDBNull(ByVal str As Object) As String
  If TypeOf str Is DBNull Then
    Return ""
  Else
    Return str
  End If
...
Function dbGetSingle(ByVal sql As String) As Object
  ...
  'Catch ex As Exception
   : if err.number<>0 then MsgBox("Fehler bei der Abfrage:" + vbNewLine + sql + vbNewLine + vbNewLine + err.description)
  'End Try
End Function
...
Function dbGetReader(ByVal sql As String, optional traceMode as boolean =false) As OleDb.OleDbDataReader
  Dim cm2 As New System.Data.OleDb.OleDbCommand
  if traceMode=true then 
    trace sql
  end if
  ...
  cm2.Connection = conADR
  cm2.CommandText = sql
  : dim RESULT As OleDb.OleDbDataReader = cm2.ExecuteReader(CommandBehavior.Default)
  if err.number<>0 then MsgBox("Fehler bei der Abfrage:" + vbNewLine + sql + vbNewLine + vbNewLine + err.description)
  return RESULT
...
Function dbExecuteSQL(ByVal sql As String) As Integer
  ...
  cm2.Connection = conADR
  cm2.CommandText = sql
  : dim RESULT as integer=cm2.ExecuteNonQuery()
  : if err.number<>0 then MsgBox("Fehler bei der Abfrage:" + vbNewLine + sql + vbNewLine + vbNewLine + err.description)
  return RESULT
End Function
...
Sub openDBConnection()
  ' ???
  if NOT conADR is Nothing then exit sub
  trace "openDBConnection()", dbFileSpec_ADR
  If dbFileSpec_ADR() = "" Then MsgBox("bitte Pfad zur Datenbankdatei angeben") : Exit Sub
  
  
  ''msgbox(dbFileSpec_ADR)
...
'-->
'--> CLASS:    cls_configPara

Shared globConfig As New cls_configPara()


Public Class cls_configPara
...
Sub appendDefaultPara(ByVal fileSpec As String)
  Dim DefPara As Dictionary(Of String, String) = DefaultValuesForConfigFile()
  Dim SW As IO.StreamWriter

  dim k As object
  For Each k In DefPara
    If config.ContainsKey(k.Key) Then Continue For

    If SW Is Nothing Then SW = IO.File.AppendText(fileSpec)
    config.Add(k.Key, k.Value)
    SW.WriteLine(FormatParaLine(k.Key, k.Value))
  Next
...
Function FormatParaLine(ByVal varName As String, ByVal varContent As String) As String
  Dim lin As String = ""
  If varName.Length < 30 Then lin += RSet(varName, 30) Else lin += varName
  lin += ": " + varContent
  Return lin
End Function
...
Sub ReadConfigFile(ByVal fileSpec As String)
  On Error Resume Next
  _fileSpec = fileSpec
  If Not IO.File.Exists(fileSpec) Then
    IO.File.WriteAllText(fileSpec, "")
  End If
  Dim fileContent() As String = IO.File.ReadAllLines(fileSpec)

  config.Clear()
  For Each line As String In fileContent
    If line.Trim = "" Then Continue For
    If line.Trim.StartsWith("//") Then Continue For
    Dim parts() As String = Split(line, ":", 2)
    config.Add(parts(0).Trim, parts(1).Trim)
  Next

  appendDefaultPara(fileSpec)
End Sub


: Default ReadOnly Property Para(ByVal index As String) As String
:   Get
:     'Stop
:     If config.ContainsKey(index) Then
:       Return config(index)
:     Else
:       Return ""
...
Public Sub New()
  If IO.File.Exists("U:\yPara\globConfig.txt") Then
    ReadConfigFile("U:\yPara\globConfig.txt")
  Else
    ReadConfigFile("C:\yPara\globConfig.txt")
  End If
End Sub
