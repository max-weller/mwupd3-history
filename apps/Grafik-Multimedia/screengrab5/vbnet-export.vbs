...
Sub OutputClass(classObject)
  ...
  For i = 0 To classObject.AttributeCount - 1
    ...
    out.WriteLine "  " + GetVisibility(attr.Visibility)+ " " + attr.Name + " As " + attr.Type
  Next
  out.WriteLine "  "
  out.WriteLine "  "
  ...
  For Each pName In properties.Keys'i = 0 To properties.Count - 1
    ...
    out.WriteLine "  " + GetVisibility(pItem.Visibility)+ IIF(pStat = 1, "ReadOnly", "")+" Property " + Mid(pItem.Name, 5) + "() As " + pTyp
  ...
  For Each meth In methods.Items
    If Not IsProperty(meth.Name) Then
      ...
      If meth.IsVoid Then
        out.WriteLine "  " + GetVisibility(meth.Visibility)+ " Sub " + meth.Name + "(" + GetParameters(meth) + ")"
        out.WriteLine "    ' to be implemented ..."
        out.WriteLine "    "
        out.WriteLine "  End Sub"
      Else
        out.WriteLine "  " + GetVisibility(meth.Visibility)+ " Function " + meth.Name + "(" + GetParameters(meth) + ") As " + meth.ReturnValue
        out.WriteLine "    ' to be implemented ..."
        out.WriteLine "    "
